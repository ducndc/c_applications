/*
 * Event loop
 * Copyright (c) 2023-2024, Chung Duc Nguyen Dang <ducndc99@gmail.com>
 *
 * This software may be distributed under the terms of the BSD license.
 *
 * This file defines an event loop interface that supports processing events
 * from registerd timeout (i.e., do something after N seconds), sockets (e.g., a
 * new packet avaiable for reading), and signals. eloop.c is an implementation of
 * most UNIX/POSIX systems. When porting to other operating systems, it may be 
 * necessary to replace the implementation with OS specific mechanisms.
 */
...
/*
 * ELOOP_ALL_CTX - eloop_cancle_timeout() magic number to match all timeouts
 */
 ...
 * @EVENT_TYPE_EXEPTION: An exeption has been reported
 */
typedef enum {
    EVENT_TYPE_READ = 0,
    EVENT_TYPE_WRITE,
 ...
 * @sock_ctx: Registered callback context data (user_data)
 */
typedef void (*eloop_timeout_handler)(int *eloop_data, void *user_ctx);

/*
 * eloop_signal_handler - eloop signal event callback type
 * @sig: Signal number
 * @signal_ctx: Registered callback context data (user_data from eloop_register_signal(),
 * eloop_register_signal_terminate(), or eloop_register_signal_reconfig() call)
 */
...
/*
 * eloop_register_read_sock - Register handler for rad events
 * @sock: File descriptor number for the socket
 ...
 * Return: 0 on success, -1 on failure
 *
 * Register a read socket notifier for the given file descriptor. The handler function
 * will be called whenever data is avaiable for reading from the socket. The handler function
 * is responsible for clearing the event after having processed it is order to avoid eloop 
 * from calling the handler again for the same event.
 */
int eloop_register_read_sock(int sock, eloop_sock_handler handler, void *eloop_data, void *user_data);

/*
 * eloop_unregister_read_sock - Unregister handler for read events
 * @sock: File desciptor number for the socket
 *
 * Unregister a read sock notifier that was registered with eloop_register_read_sock().
 */
 ...
 * Return: 0 on success, -1 on failure
 *
 * Register an event notifier for the given socket's file descriptor. The handler function
 * will be called whenever the that event is triggered for the socket. The handler function 
 * is responsible for clearing the event after having processed it in order to avoid eloop
 * from calling the handler again for the same event.
 */
